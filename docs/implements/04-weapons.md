...
## 관측 고정 (zeno — 2026-09-14 제거됨)

같은 대상을 지속 조사하면 감속하다 동결되던 기제. **코드·데이터·집계·보상·테스트를 모두 제거했다** —
`ZenoSpec` / `applyZeno` / `zenoExposureStep` / `zenoSlowMul` / `isZenoFrozen` / `zenoLatch` /
`stats.zenoFreezes` / `tests/zeno.test.ts` 모두 존재하지 않는다. 무기 JSON 의 `zeno` 블록도 없다.

제거 이유와 대체 방향(특수무기 **무력화** · 보스 무력화 패턴)은 [13-무력화 체계(예정)](../spec/13-neutralize.md).
요점만: 자동사격이 명중을 계속 넣으므로 "맞히기 유지"가 조건인 기제는 플레이어의 선택이 되지 못했고,
동결 횟수를 세던 채점이 기체 간 비대칭을 만들었다.

빔에 남은 관측 계열 효과는 **수동 전용**인 관측 펄스(`manual.decohere`)·관측 계류(`manual.pinSec`) 둘이다.
