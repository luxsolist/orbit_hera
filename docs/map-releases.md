...
서울·부산·로마·아테네에 적용한다. 원본은 GitHub Releases, 게임용 2×2 압축 묶음은 Git에 보관한다. 일반 청크(서울·부산 각 1,600개, 로마·아테네 각 1,640개)는 Git 추적에서 제외하고 로컬 파일은 유지한다. 수작업·교정 내역을 검토할 수 있도록 랜드마크 상세, 외벽, 도로 보정 및 도시 목록은 Git에도 남기고 원본 백업과 게임용 묶음에 함께 포함한다.

| 도시 | 원본 셀 | 묶음 | 게임용 압축 용량 | 원본 백업 |
|---|---|---:|---:|---:|
| 서울 | 37/126 | 400 | 16.34MB | 3,596파일 / 16.19MB |
| 부산 | 35/129 | 420 | 4.76MB | 2,396파일 / 4.75MB |
| 로마 | 41/12 | 441 | 13.78MB | 2,828파일 / 13.79MB |
| 아테네 | 37/23 | 441 | 16.12MB | 2,430파일 / 16.13MB |
...
이하 접미사가 없는 복원·게시 명령은 서울용이다. `maps:verify`와 빌드 전 검증은 등록된 모든 도시 검사하며 기본 `npm test`는 등록된 도시의 누락 원본을 자동 복원한다. Python 도구를 직접 사용할 때는 `--city busan`을 붙인다.
...
## 배포
`npm run build`는 원본 재생성 대신 게임용 묶음과 Releases 포인터 일치를 검사한다. 따라서 서울·부산·로마·아테네 원본 없이 빌드할 수 있다. 생성 압축 파일은 같은 서버 경로에서 서빙하므로 플레이 중 GitHub Releases에 직접 접속하지 않는다. 원본이 없는 환경에서 지도 묶음이 실패하면 재시도하고 오류를 보고한다.

현재 변경은 Git의 최신 트리에서 원본을 제외한다. 과거 커밋에 남은 지도 용량은 줄이지 않으며, 이력 재작성은 수행하지 않는다. 서울·부산·로마·아테네 외 도시는 기존 방식을 유지한다.
...
압축·백업·복원·전송은 도시별 구현을 만들지 않고 공통 도구를 사용한다. `config/map-cities.json`에 도시 ID와 원본 셀을 등록한다. 예: `"rome": [41, 12]`. 실제 `tiles.json`의 셀과 원본 파일을 먼저 확인한다. ID는 영문 소문자로 시작하며 소문자·숫자·하이픈만 사용한다.

1. 원본 청크와 `tiles.json`, 필요한 상세/도로/외벽 보정을 준비한다.
2. 등록 파일에 도시를 추가한다. 동일 셀을 중복 등록하면 실패한다. 현재 원본 구조가 셀을 키로 사용하므로 같은 셀의 도시는 별도 두 묶음으로 등록하지 않고 데이터 통합 방식을 먼저 정해야 한다.
3. `npm run build:map-bundles -- <도시>`로 압축과 인덱스를 생성한다. 원본 블록 크기는 `tiles.json.block`을 따르고 전송 묶음은 공통 2×2다. 음수 좌표도 내림 나눗셈으로 처리한다.
4. `npm run maps:prepare:city -- <도시>`로 원본/묶음 일치 검증 및 로컬 백업을 생성한다. 이 명령은 게시하지 않는다.
5. 관련 검증 후 `npm run maps:publish:city -- <도시>`로 실제 게시·재다운로드 검증을 수행한다. 기존 릴리스는 덮어쓰지 않는다.
6. 새 도시 원본 복원을 별도 작업 폴더에서 검증한 다음 해당 셀 원본만 Git 추적에서 제외하고 `.gitignore`에 경로를 추가한다. 원본을 먼저 삭제하지 않는다. `tiles.json` 및 수작업 보정 데이터는 Git에 남긴다. 이 단계는 공통 도구가 자동 실행하지 않는다.
7. 등록 파일·인덱스·압축·게시 포인터·관련 코드·추적 제외를 함께 커밋한다. 신규 등록만 하고 인덱스를 생성하지 않으면 앱이 설정 오류를 보고하며, 게시 포인터가 없으면 배포 검증이 실패한다.

| 공통 명령 | 역할 |
|---|---|
| `npm run build:map-bundles -- <도시 또는 all>` | 원본에서 묶음 생성 |
| `npm run maps:prepare:city -- <도시 또는 all>` | 원본 검증 및 로컬 백업 생성 |
| `npm run maps:publish:city -- <도시 또는 all>` | 도시별로 재생성 후 게시·재다운로드 검증 |
| `npm run maps:restore:city -- <도시 또는 all>` | 누락된 원본만 복원 |
| `npm run maps:verify -- <도시 또는 all>` | 게시 포인터·묶음 해시·전체 청크 포함 여부 검증 |

`all`은 현재 등록된 도시만 순차 처리한다. 실패 시 중단하며 앞서 완료한 도시를 롤백하지 않는다. 같은 명령을 재실행하면 동일 릴리스는 검증 후 재사용한다. `npm test`도 등록 목록 전체의 원본 누락 여부를 검사한다. 기존 서울/부산 명령은 공통 구현을 호출하는 호환 별칭이다.

런타임 `MapBundles.ts`는 등록 목록과 `*-bundles.json`을 자동 연결한다. 신규 도시마다 import나 캐시 경로를 추가하지 않는다. 해시 파일의 장기 캐시도 모든 도시 ID에 동일하게 적용한다. 부분 묶음에 존재하지 않는 청크는 묶음 대상이라고 판단하지 않는다.

`MapCorrections.ts`는 전송 방식과 무관한 지형 보정 경계를 담당한다. 공통 도로 보정은 모든 도시에 적용하고, 부산의 상세→도로 및 서울의 도로→상세 순서는 유지한다. 새로운 도시의 특수 모델/지형 해석이 필요한 경우에만 이 보정 계층에 추가한다. 데이터를 압축해 보관하는 것만으로 새 종류의 랜드마크 모델이 자동 생성되지는 않는다.

이번 공통화에서 서울·부산 묶음을 재생성한 결과 기존 게시 인덱스와 동일했다. 지도 내용과 Releases 버전 변경은 없다. 가상 신규 도시의 음수 좌표·8청크 원본 블록·부분 묶음·누락 검출·중복 셀 거부를 포함해 관련 테스트 41개와 타입 검사를 통과했다.

## 로마 적용

로마 원본 1,640개와 랜드마크/도로 보정을 441개 2×2 묶음으로 전송한다. 격자 경계에서는 부분 묶음이 생긴다. 직렬화 데이터 46,766,040바이트 → 압축 13,777,093바이트(약 70.5% 감소). 원본 백업은 2,828파일, 13,785,355바이트다.

- [초기 로마 Releases](https://github.com/luxsolist/orbit_hera/releases/tag/maps-rome-a3138178d3ea7268); 최신 기준은 `config/map-releases/rome.json`.
- `npm run maps:restore:city -- rome`: 누락 원본 복원.
- `npm run maps:publish:city -- rome`: 재생성·일치 검증·게시·재다운로드 검증.
- 로마 전용 로더나 CLI를 추가하지 않고 등록 파일에 `[41,12]`를 추가했다.
- 상세 데이터의 add 및 romeSites도 통째로 포함하므로 분수 추가 형상, 계단, 교량, 기념물과 유적 표면이 유지된다. 절차식 모델 생성 코드는 기존처럼 Git에서 관리한다.
- 원본 1,640파일은 로컬에 남고 Git 추적만 제외된다. 별도 폴더 복원과 기존 파일 보존, HTTP 압축 파일 해시를 확인했다. 관련 테스트 28개 및 타입 검사 통과.

## 아테네 적용

원본 1,640청크를 441개 2×2 묶음으로 전송한다. 음수 좌표 및 경계의 부분 묶음을 포함한다. 랜드마크 상세 6청크와 도로/외벽 보정을 같은 묶음에 포함한다.

- 직렬화 데이터 60,641,727바이트 → 압축 16,124,736바이트(약 73.4% 감소).
- 원본 백업 2,430파일 / 16,130,090바이트. 지도 원본과 상세/보정 데이터를 포함하며 모델 TypeScript 코드는 Git에서 별도 관리한다.
- [최초 아테네 Releases](https://github.com/luxsolist/orbit_hera/releases/tag/maps-athens-0c81318c1c9460ae). 최신 기준은 `config/map-releases/athens.json`.
- 원본 청크는 로컬에 유지하고 Git 추적에서 제외한다. 압축 묶음·인덱스·포인터·상세/보정 파일은 Git에서 관리한다.
- `npm run maps:restore:city -- athens`: 누락된 원본 복원. 기존 파일은 덮어쓰지 않는다.
- `npm run maps:publish:city -- athens`: 수정 후 2×2 재생성, 원본 동등성 검사, 새 릴리스 게시 및 다운로드 검증.
- 게시된 파일의 해시 검증, 빈 임시 폴더에 2,430파일 복원, 재복원 시 기존 편집 파일 보존을 확인했다.
