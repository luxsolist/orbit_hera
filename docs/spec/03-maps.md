...
#### 자동 승격 차단(2026-08-27) — 배제에 **강제력**을 준다

위 배제는 "큐레이션 배열에서 뺀다"였는데, 그것만으로는 **효과가 없었다**. 랜드마크는 큐레이션
말고도 `landmarkFrom()` 태그+면적 자동 승격으로 들어오기 때문이다(실측: 도시당 큐레이션 8~26 대
자동 승격 2~844 — 정책이 관장하던 범위는 전체의 1~10%였다). 손으로 뺀 대상이 OSM 이름을 달고
그대로 되돌아온다. 실증 2건:

- **베이징** — 천안문광장을 뺐으나 그 광장 안 `毛主席纪念堂`(Chairman Mao Memorial Hall)이
  `historic=tomb` → `deep-roots` 로 승격돼 **배포본에 실려 있었다**.
- **바그다드** — 배제한 '승리의 손'이 OSM `سيوف القادسية` **2채**(한 쌍이라 2개 way)로 승격 예정이었다.
  빌드 전 감사에서 포착.

그래서 `landmark-catalog.json` 에 `blockedLandmarks` 를 두고 빌드가 강제한다
([`applyBlocklist`](../../scripts/osm.mjs) ← [`build-maps.mjs`](../../scripts/build-maps.mjs)).

| 항목 | 값 |
|---|---|
| 매칭 | **좌표**(포함 ∪ 반경 40m). 이름이 아니다 |
| 적용 시점 | 큐레이션 승격 **직후** — 큐레이션·자동 승격 둘 다 덮는다 |
| 효과 | 그 건물의 `lm`·`n` 을 **함께** 제거(일반 건물로 강등). `n` 만 남으면 표시명이 청크에 실린다 |
| 다중 매칭 | 한 좌표에 걸리는 건물을 **전부** 차단 — 궁전 단지에서 한 채만 막으면 나머지가 남는다 |
| 반경 조정 | 항목에 `radiusM` 을 주면 그 값을 쓴다 — **경내 전체가 대상**인 경우가 있다(아래) |

**경내 단위 차단 — 야스쿠니가 보여준 함정.** 좌표만 넣으면 되는 게 아니다. 야스쿠니는 본전 좌표
40m 안에 **승격 건물이 하나도 없다**(본전 562㎡·배전 740㎡ 는 `amenity=place_of_worship` 가 개별
건물 way 에 없어 분류 자체가 안 되고, ritual 문턱 800㎡ 에도 못 미친다). 실제로 승격되는 것은
**76m 의 노가쿠도**(resonance 528㎡)와 **113m 의 유슈칸**(archive 5,226㎡ — 신사 부속 전쟁박물관)이다.
기본 반경으로 넣었다면 **아무것도 막지 못했을 것**이다(도쿄 추출 실측 2026-08-27).

그래서 `radiusM: 200` 으로 경내를 덮는다. 넓힐 때는 **그 반경 안에 남의 승격 건물이 없는지 먼저
실측**할 것 — 과잉 차단은 조용히 일어난다. 야스쿠니는 400m 안에 소속 외 승격 건물이 없음을 확인했고,
실제 도쿄 데이터로 왕복 검증했다(승격 407 → 405, 야스쿠니 2채만 제거, 미적중 0).

**이름이 아니라 좌표로 매칭하는 이유**는 둘이다. ① 승격은 위치로 결정돼야 같은 청크를 어느 도시가
굽든 같은 결과가 나온다(전역 레지스트리 원칙 — §랜드마크 승격). ② 이름 전역 차단은 동명 충돌을
일으킨다 — "Peace Palace" 는 헤이그 국제사법재판소이기도 하다.

차단은 `applyCuratedLandmarks` **밖**에서 돈다. 안에 두면 그 함수의 조기 반환(`catalogCity` 없음 ·
카탈로그 읽기 실패 · 도시 미등재)에 가려 손 맵과 무카탈로그 맵에서 조용히 건너뛰어진다.

#### 전수 감사 — `npm run audit:landmarks`

차단 목록은 **감사가 있어야 자란다**. [`audit-landmarks.mjs`](../../scripts/audit-landmarks.mjs) 는
큐레이션 카탈로그가 아니라 **배포 청크**를 훑는다 — 랜드마크의 대다수가 자동 승격으로 들어오므로
카탈로그만 보면 실제 배포물을 못 본다. 다국어 어휘로 5범주 후보를 뽑고 **판정은 하지 않는다**.
과잉 포착이 설계 의도다(놓치는 쪽이 훨씬 비싸다).

첫 전수(2026-08-27 · 27개 맵 · 승격 6,190건 → 후보 219건)에서 확인된 것:

- 후보 상당수가 **오탐**이다 — `المستنصرية`(무스탄시리야 대학)가 `نصر`(승리)에, `Santa Cecilia`가
  `cecil`(세실 로즈)에, `승리교회`·`الإمامين العسكريين`(알아스카리 성원)이 각각 걸린다. 정상이다.
- 궁전 104건은 대부분 유적(톱카프·경복궁·자금성 전각·팔라초 군)이라 그대로 둔다. 정책이 일반
  국가의 정부청사를 명시적으로 예외 처리하므로 아테네 대통령 관저·팔라초 키지(이탈리아 총리실)·
  카이로 대통령궁도 유지 대상이다.
- **놓쳤던 차단 대상 7건**을 잡았다. 1차 감사가 way 만 훑고 **릴레이션을 빠뜨린** 탓이다 —
  다마스쿠스 대통령궁(`relation/71797`, 아사드 정권의 본궁)·티슈린 궁(`relation/71812`)·영빈궁,
  바그다드 아드난 궁. 릴레이션 누락은 베이징에서 한 번 고쳤던 실수를 다른 도시에 재적용하지
  않아 남은 것이었다.
- 승격 문턱을 넘지 않은 후세인 궁전 3채(공화국 궁전·수주드 궁·자파라니야 궁)는 **선제 차단**했다.
  현재 `building=yes`/`building=public` 이라 미승격이나 `historic`/`castle_type` 태그가 하나만
  붙으면 즉시 승격된다. 좌표에 건물이 존재하므로 미적중 경고도 나지 않는다(공짜 보험).

**판단 보류로 남긴 것**(정책이 곧장 답을 주지 않아 사람 결정이 필요): 베이징 중국공산당 역사전람관·
중국인민혁명군사박물관·향산혁명기념관(기관은 `archive` 이나 천안문광장·마오 기념당을 배제한
맥락과 이어진다), 홍콩·선전 Government House.

2차 전수(1장 완결분 4개 맵 · 승격 615건 → 후보 12건)는 **위반 0건**이었다. 판단이 필요했던 둘:

- **Adwa Victory Memorial Museum**(아디스아바바) — `승리` 어휘에 걸리지만 유지한다. 1896년 아드와는
  에티오피아가 **식민 침략을 격퇴한** 사건이고, `war-glorification` 이 겨냥하는 것은 가해자의 전승
  과시다. 방어한 쪽의 기념관이라 성격이 반대다(바그다드 '승리의 손'과 대비).
- **National Palace**(아디스아바바) · **Palacio Nacional**(멕시코시티) — 정상 국가의 정부청사라
  정책의 명시적 예외에 해당한다(아테네 대통령 관저·카이로 대통령궁과 같은 처리).

#### 도시 단위 빌드 제외 — `buildExcluded`

개별 항목이 아니라 **도시 자체**를 전장으로 삼지 않는 판단이 필요할 때 쓴다. `city-names.json` 에
`buildExcluded: { decidedAt, category, reason }` 를 달면 `gen-city-config` 가 카탈로그로 통과시키고
`maps.config` 의 `generatedMaps()` 가 `MAPS` 에서 뺀다 — 빼야 `build-pipeline` 과 `--all` 전량 빌드
양쪽에서 실효가 있다. **카탈로그에는 남긴다**: 장 배속·100선 집계가 유지돼야 하고, 빈자리는 누락과
구분되지 않기 때문이다.

첫 적용(1건): **예루살렘**(dispute-symbol, 2026-08-27). 개별 항목 단위로는 위반이 없었다 — 자동 승격
124건의 `memorial` 4건은 야드 바셈 추모관·전몰자 추모관·케네디 기념비로 정책이 명시적으로 유지
대상이라 못박은 범주이고, 정부청사는 승격 목록에 없었다. 도시를 뺀 근거는 둘이다: 성전산/하람
알샤리프의 귀속이 분쟁의 핵심 쟁점인데 랜드마크는 `guard`/`aggro:landmark` 미션에서 실제로 피격·
붕괴하고(`destroyedLandmarks`), 반경 20km 커버리지가 그린라인을 가로질러 라말라(북 13.5km)·
베들레헴(남 9km)을 한 전장에 담는다.
...
### OSM 수집 — **Geofabrik 추출 전용**(유일 경로)

**Geofabrik 지역 추출(.osm.pbf)에서 bbox 만 잘라 한 번에** 확보한다. 절차:
...
#### Overpass 수집 경로 제거(2026-08-27)

`build-maps` 는 추출 캐시(`/tmp/osm-<id>.ndjson`)를 **읽기만** 한다. 캐시가 없으면 위 절차를 안내하며
**즉시 실패**한다. 옛 Overpass 1km 타일 폴백은 제거했다 — 근거 넷:

1. **조용한 오염 위험이 실재했다.** `overpass.osm.ch` 는 요청 bbox 와 무관하게 늘 같은 스위스 지역
   응답을 돌려주는데(2026-08-22 진단이 주석에 남아 있었다), 수집기는 `elements.length>0` 이면
   채택했다. 앞 엔드포인트가 실패하면 도쿄 맵에 스위스 지형이 조용히 구워진다 — 엔드포인트 **순서**
   로만 막고 있었으니 방어가 아니라 운이었다.
2. **속도가 70배 차이났다.** 실측 루앙프라방 1,558타일 · 약 8타일/분 → 3시간+ 대 추출 경로 **2분 26초**.
3. **단일 쿼리 분기는 죽은 코드였다.** bbox ≤ 0.06° 일 때만 타는데 등록된 104개 맵 중 해당 0개.
4. **폴백이 진짜 문제를 가렸다.** 캐시가 없을 때 필요한 건 "추출을 먼저 돌려라"는 즉각적 실패다.
   조용히 3시간짜리 우회로로 빠지면 원인을 못 본다(실제로 한 번 빠졌다).

가용성도 근거를 뒷받침한다 — 같은 쿼리로 4개 미러를 재보니(2026-08-27) `overpass-api.de` XML 에러 ·
`private.coffee` 40초 타임아웃 · `kumi.systems` 빈 응답 · `osm.ch` 만 응답(위 오염 사례). 상시 불통은
아니고 **간헐적**이지만, 빌드 재현성을 여기에 걸 이유가 없다.

함께 사라진 것: `osm.bboxTiles` · `osm.mergeOSM` · `osm.overpassQuery`(전부 이 경로 전용) · `OSM_PARTIAL`
중간 빌드 모드. 수역만 다시 받는 일회성 수리 도구 [`refetch-water.mjs`](../../scripts/refetch-water.mjs)
는 **자체 Overpass 경로**를 갖고 있어 영향받지 않는다(같은 불안정성은 그대로 안고 있다).
...
재현성은 **추출 파일을 고정**해서 얻는다. Geofabrik 은 `-latest` 외에 날짜가 박힌 스냅샷을 함께
publish 하므로(`laos-latest` → `laos-260824.osm.pbf` 로 리다이렉트), 그 날짜 파일을 받아 두면 같은
입력에 같은 결과가 나온다. 같은 `.osm.pbf` + 같은 `import-extract` = 같은 `/tmp/osm-<id>.ndjson`.

> 옛 `OSM_DATE` 환경변수(Overpass `[date:]` 스냅샷)는 **동작하지 않는다** — Overpass 수집 경로와 함께
> 제거됐다(2026-08-27).
