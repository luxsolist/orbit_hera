...
- 패턴군 A(군집)가 Ⅰ단계 기본 소비, B~C 가 중반 숙련(직무 식별 훈련 — 6번이 튜토리얼 성격),
  1막 클라이맥스 미션([서사편 §5] ⚠️)은 재설계 대기 — 종전 D-13(봉합전)은 승리 조건이 관측 고정
  게이지였어서 그 기제와 함께 제거됐다(2026-09-14, [13-무력화 체계](13-neutralize.md)).
  E 는 1막 전반에 산재(브리핑이 세계관 계시 통로).
...
| `ritual` | 의례의 자리 | **반복 측정의 안정화** — 같은 의례의 반복이 자리를 붙든다 | `amenity=place_of_worship`, temple/church/mosque | 저항 ×1.6(최고) · 디졸브 저항 |
