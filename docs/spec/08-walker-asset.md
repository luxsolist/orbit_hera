...
- 고품질 버전: 7,080 triangles, 35 mesh batches, 18 groups, 6 sockets. 그림자 패스의 draw call은 별도다.
...
전진 40km/h(11.111m/s), 좌우·후진 20km/h(5.556m/s), 전 방향 대시 300km/h(83.333m/s), 점프 이륙 속도 100km/h(27.778m/s). 대시는 가속 240m/s², 추진 0.65초, 제동 300m/s², 재사용 2초다. 정지 상태 기준 최고속도까지 약 0.347초, 추진 거리 약 39.7m, 완전 제동 거리 약 11.6m다. 지상 응답 계수 4/s, 공중 2/s. 점프 상승 중력 28m/s², 하강 36m/s², 하강 종단 40m/s로 단일 점프 높이 약 13.8m, 체공 약 1.87초이며 공중 재점프를 유지한다.

고속 주행에서는 보행 진폭을 줄이고 방향 노즐의 지속 추력으로 이동을 보조한다. 발 디딤 주기는 초당 2.5회 이내로 유지하며, 대시 감속 중에는 진행 방향 쪽 노즐에서 역추력을 분사한다.

## 지상 달리기 개정

최신 지상 속도는 전진 50km/h(13.889m/s), 좌우·후진 25km/h(6.944m/s)다. 일반 지상 이동의 추진 보조 분사는 제거했다. 속도에 따라 보폭, 발 들기 높이, 접지 비율과 몸체 반동을 조절한다. 접지 중 발의 후퇴 속도를 실제 이동 속도에 맞추고 비접지 구간은 부드럽게 회수하며 최대 초당 3.6회 디딤으로 제한한다. 측면 보폭은 다리 교차 방지를 위해 제한한다. 대시 300km/h, 점프 이륙 100km/h 및 대시·점프·공중 이동 분사는 유지한다.

### 좌우 이동 속도 조정

일반 좌우 이동 최고속도를 4m/s(14.4km/h)로 낮췄다. 전진 50km/h, 후진 25km/h와 전 방향 대시 300km/h는 유지한다.

### 청색 애프터버너 분사

모든 추진 노즐은 곧고 가느다란 청색 분사로 표시한다. 청백색 중심에서 파란 외곽·끝으로 색이 변하며 길이와 가장자리 방향으로 투명하게 사라진다. 저불투명도 가산 합성과 약한 청색 반사광을 사용하며 주황색 불꽃은 제거했다.

## v12 · 렌즈형 에너지빔 무장

양팔의 물리 총신을 제거하고 기존 무기 외피의 후면 기준으로 앞뒤 길이를 1.5배 연장했다. 전면 원형 렌즈와 금속 테두리를 배치하며 좌우 빔 소켓은 각각 렌즈 중심의 앞면에 정렬한다. 렌즈는 sensor 재질 슬롯을 공유해 스킨 발광색을 따른다.
