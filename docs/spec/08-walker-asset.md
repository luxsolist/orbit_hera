# ANDROID-01 v11 · ED-209형 공용 역관절 워커
...
로보캅(2014) ED-209 참고 이미지의 실루엣을 바탕으로 긴 쐐기형 일체 차체, 양옆 쌍열 포신, 측면·하단 추진구, 뒤로 꺾이는 무릎과 긴 정강이·갈라진 발을 구성한다. 사람형 머리와 손은 없다. 안정적인 에셋 ID android-01과 walker 연결은 유지하며 버전은 7이다.
...
- 고품질 버전: 7,136 triangles, 35 mesh batches, 18 groups, 6 sockets. 그림자 패스의 draw call은 별도다.
...
형상·관절과 재질을 분리하고 6개 재질 슬롯, JSON 스킨 카탈로그, 즉시 교체 및 선택 스킨 GLB 내보내기를 지원한다. 상세 계약과 예제는 [워커 스킨 에셋](10-walker-skins.md)을 참조한다.

## v5 기본 외피와 장식 분리

기본형은 무채색 골격·외피만 표시한다. 표면 패널, 방열 슬릿, 표식, 볼트는 `WalkerGarnish`가 스킨 소유 메시로 각 관절에 부착한다. 스킨 해제 시 장식 형상과 재질 도색을 제거한다. 기본 GLB도 장식 없는 형태로 제공한다.

## v6 전면 형상 정리

눈썹형 발광부와 원형 전면 센서·받침을 제거하고 쐐기형 외피를 유지한다. head 노드는 카메라 소켓과 애니메이션 호환을 위한 비가시 기준점으로 남긴다. 전면 추진구는 하단 양옆의 납작한 사각 슬롯이며, 측면 앞 노즐 주변 발광 스트립과 검은 받침은 제거했다.

## v7 동체 길이 조정

동체의 앞뒤 축(Z) 스케일을 0.82에서 0.984로 20% 늘렸다. 너비·높이·다리·포신 치수는 유지하고, 스킨의 동체 장식은 동일한 비율 설정을 공유한다. 측면 및 하단 노즐의 전후 위치도 같은 비율로 정렬하며 전면 노즐은 늘어난 외피 표면에 부착한다.

## v8 동체 길이 추가 조정

현재 동체 앞뒤 길이에서 20%를 추가해 Z 스케일을 0.984에서 1.1808로 변경했다. 최초 0.82 대비 총 44% 길어진 비율이며, 스킨 장식과 노즐 위치도 동일한 비율을 따른다.

## v9 독립 골반 프레임

양쪽 다리는 골반의 고관절에 연결되고 몸통은 골반 위 회전 베어링으로 지지된다. 몸통 회전축은 고관절보다 0.70m 높아 조준 시 외피와 고관절의 간섭을 방지한다. 하방 점프 노즐은 골반 중앙 아래에 부착되어 몸통 조준 회전과 독립적으로 움직인다.

## v10 밀착형 몸통 연결

노출된 원통형 회전 베어링 3개를 제거하고 몸통 회전축 높이를 골반 기준 0.70m에서 0.54m로 낮췄다. 골반과 몸통은 좁은 회전 여유만 남기며, 골반에 연결된 다리와 하방 노즐의 구조는 유지한다.

## v11 골반 연결 하우징

몸통과 골반 사이 빈 공간을 낮은 다면체 장갑 하우징으로 채웠다. 중앙의 좁은 연결 코어는 몸통 하부 안쪽으로 들어가며, 골반 하우징 바깥에 원통형 베어링이 노출되지 않는다. 이 하중 지지 구조는 기본 골격에 포함되며 스킨의 장갑 및 프레임 재질을 공유한다.

## 인트로 기본형 공유

인트로의 출동(safety) 및 조종실(link) 장면은 `CinematicWalker`를 통해 최신 `WalkerMech` 기본형과 `WalkerThrusters`를 함께 생성한다. 스킨 장식은 적용하지 않으며, 골반 하방 노즐을 포함해 미리보기와 같은 기능 부품을 사용한다. 장면 해제 시 노즐 리소스를 먼저 해제한 뒤 본체를 해제한다. 조종실 카메라는 현재 모델의 전신이 들어오도록 배치한다.
