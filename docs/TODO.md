...
### Phase 1 — 전투 재정립의 척추 (서사편 §6.10-1 × §7.5-1)

- [ ] 소인체(`marker`) 워커 + 낙인 상태 + 심판 파문(전장 이벤트) — 기존 아키타입과 병행 투입으로 시작. 예상 공수: 3~5일
- [ ] W1 관측 고정(`zeno`) — 빔 지속 조사 = 시전 인터럽트. 예상 공수: 1~2일
...
### Phase 3 — 대위상 세트 (물리편 §2.1+§2.2+§2.7.1 + 서사편 §7 W2)

- [ ] 위상 이탈(스펙 `phase` 블록 + 상태기계) → 관측 펄스(`manual.decohere`) → 중력 렌즈 왜곡 → W2 관측 계류. 워커=대위상 앵커 / 플라이어=타이밍 타격 완성. 1주

### Phase 4 — 인식 Ⅱ 계시 (중간 반전 — 서사편 §1.10 계시 설계)

- [ ] 택티컬 미션 게임 배선(순수 코어 `tactical.ts` → GameInstance/스폰/HUD 연결) — 기술적 핵심. 3~5일
- [ ] 동시 타격 실험 미션 + 필라멘트(DrainBeams 재활용) + 궤적 타임랩스 컷씬. 3~5일
- [ ] 다중 투영 미니보스(HP 공유 N구체) + 점수판 문법 전환 + 명칭 갱신 연출(§8.3). 3~5일

### Phase 5 — 워커 로스터 완성 + 시간축

- [ ] 절단체(`cutter`) 건물 납치 + W4 복구 사격(방어 미션의 새 동사 세트). 1주
- [ ] 역행체(`rewinder`) 되감기 + 예지 HUD(2.8.1) — 상태 링버퍼 인프라(링크 리와인드 2.8.3 과 공유). 1주
...
- §7.4 진행 시스템 MVP — Phase 2 저장 인프라와 동일 기반, 묶어서 진행
