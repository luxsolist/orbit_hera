...
## ✅ 해결됨 (2026-08-23) — "모든 미션이 사냥 하나로 느껴진다"

실플레이 피드백: 목표가 6종으로 분화돼 있는데 체감이 전부 같다. 진단 결과 **미션 스키마가 아니라
런타임이 모든 목표를 같은 행동으로 수렴**시키고 있었다 — 승리 조건은 "언제 끝나는가"만 바꾸고
"무엇을 해야 하는가"는 안 바꿨다.

원인 넷(앞의 둘은 코드가 아니라 **의도한 설계가 무력화된 것**):

1. **22개 중 18개가 기본 어그로(player)** — 적이 플레이어에게 오니 목표와 무관하게 루프가
   "오는 걸 죽인다"로 동일. 특히 `guard-landmark`·`hold-city` 에 `aggro` 변조가 없어 표적이
   공격받지 않았다 = 잃을 대상이 없는 생존전.
2. **`provoked` 가 영구 래치** — 주무기가 360° 자동사격이고 유발이 반경 100m 로 전파돼, 첫 교전
   몇 초 뒤 전장 전체가 플레이어만 쫓았다. 어그로 변조는 도입부에만 존재하다 사라졌다.
3. **인식 해제 히스테리시스가 변조를 무시** (테스트가 발견) — `aggro=landmark/building` 이 획득
   반경만 0 으로 막고 **해제 반경 900m 는 그대로**였다. 유발로 한 번 붙은 적은 유발이 풀려도
   근처에 있다는 이유로 계속 플레이어를 쫓았다 — 감쇠만 넣었다면 여전히 안 고쳐졌을 것이다.
4. **비표적 처치·생존 교전에 대가가 없다** — `purge-role` 에서 잡몹을 죽여도 손해가 없고,
   `survive` 에서 처치가 위협 감소 + HP 환수까지 겸하니 최적해는 늘 "전부 죽인다".

대응:

| 조치 | 내용 |
|---|---|
| 어그로 변조 보강 | `guard-landmark`→`aggro:landmark` · `hold-city`→`aggro:building` (데이터) |
| 유발 감쇠 | 영구 래치 → **10초 타이머**. 재피격은 갱신(누적 아님). 교전 중엔 유지되고, 놓으면 적이 제 임무로 복귀 |
| 해제 반경 정합 | 변조 시 획득·해제 **둘 다 0** — 유발만이 플레이어 어그로의 유일한 경로 |
| 비표적 비용 | `offTargetPenalty`(신규 변조) — 비표적 1기당 시간 차감. brand-hunt/bodyguard 6s · retro-hunt 8s. HUD 에 `비표적 N (−Ns)` 병기 |
| 생존 교전 유인 제거 | ~~`killHealMul`(신규 변조)~~ — **2026-08-25 회복 전면 폐지로 변조 자체가 제거됨**(전 미션 공통이 됐다) |

`provokeNear` 의 "이미 유발된 개체 건너뛰기"도 제거했다 — 영구 래치 시절의 조기 종료였고, 그대로
두면 계속 쏘는 적이 10초 뒤 어그로를 풀어버린다(테스트가 발견).

부수 효과: v1 어댑터(`toLegacy`)는 변조 축이 없으므로 이제 1번(정화 작전)만 v1 동치다.
검증은 [reinforce.test](../tests/reinforce.test.ts)(유발 감쇠 → 랜드마크 복귀) ·
[enemyAwareness.test](../tests/enemyAwareness.test.ts)(감쇠·갱신) ·
[missionV2.test](../tests/missionV2.test.ts)(비표적 비용) + 시나리오 하네스 `aggro`/`offtarget`.
...
| **P4** | 1막 클라이맥스 | 균열 스폰(2.4) → **무력화 체계**([spec/13](spec/13-neutralize.md) — 특수무기 교체·보스 무력화 패턴) → 클라이맥스 미션 재설계(구 봉합전 제거) → W5 공명 각인(score 골) → 사고 로그 파편 (§8 Phase 2 잔여+Phase 6) | ~3주 |
...
2026-08 재정립 1단계로 **소인체(marker — 낙인+심판 파문)·다중 투영 보스** 추가(같이 들어갔던 관측
고정(zeno)은 2026-09-14 제거 — [spec/13](spec/13-neutralize.md)),
...
- [~] W1 관측 고정(`zeno`) — 2026-08 구현 후 **2026-09-14 전면 제거**(자동사격과 충돌해 플레이어의
  선택이 되지 못했고, 동결 횟수 채점이 기체 간 비대칭을 만들었다). 대체 방향은 특수무기 **무력화**
  → [spec/13-무력화 체계](spec/13-neutralize.md)(미구현)
...
- [x] **중력 렌즈 왜곡** ✅ 완료(2026-08, §2.7.1) — [lensDistort.ts](../src/fx/lensDistort.ts)(순수
  ...
  읽는 유일한 채널**.
- [x] **미니맵 방향 핑** ✅ 완료(2026-08-24) — `Minimap.drawPhaseDirections`: 위상 개체는 위치를
  아예 안 올리고 테두리에 속이 빈 다이아몬드로 방향만 펄스 표시(랜드마크의 꽉 찬 화살표와 형태로
  구분). 여러 개체가 겹치면 `pickEdgeMarkers` 로 가까운 순 솎아낸다.
...
- [ ] 1막 보스전(마리아나 거대 코어 — 승리 조건 재설계. 구 봉합전은 관측 고정 게이지 기반이라
  기제와 함께 폐기) · 사고 로그 파편(어휘 3단계 게이트) · W5 공명 각인 · 실패 임계 → 3막 들여다봄 엔딩. **v1 캠페인(지구 완주) 완성.** 2막 행성(달/화성/목성)은 이후 확장.
...
- **무력화 체계**([spec/13](spec/13-neutralize.md)) — 드론 특수무기 2종 교체(연속발사 → 무력화) +
  보스 특수 능력의 무력화 공격 패턴. 수치·효과 종류 전부 미정. Phase 6 클라이맥스의 선결 항목.
- **미션 패턴 잔여**(spec/06-missions §3 — 시스템 도착 시 자동 편입): 4 공명 시험(score 골 — W5 와
  함께) · 14 부유 요새(1km+ 보스 콘텐츠) · 16 순례길·18 마지막
