...
export const WALKER_ASSET = {
  id: "android-01", version: 3, droneId: "walker",
...
export class WalkerMech extends THREE.Group {
  ...
  constructor(options: WalkerOptions = {}) {
    ...
    const armor = new THREE.MeshStandardMaterial({ color: options.armorColor ?? 0x202830, roughness: .57, metalness: .48, roughnessMap: this.roughness });
    const trim = new THREE.MeshStandardMaterial({ color: options.trimColor ?? 0x404d57, roughness: .5, metalness: .38, roughnessMap: this.roughness });
    ...
    this.torso.name="torso";this.torso.position.y=.08;this.pelvis.add(this.torso);
    block(this.pelvis,frame,[1.25,.30,.65],[0,0,-.15],"load_bearing_chassis");
    cylinder(this.pelvis,steel,.29,.19,[0,.15,-.12],"y");
    ...
    loft(this.torso,trim,[[-.75,.12,.80,.74],[.05,.16,.88,.78],[.91,.075,.43,.35]],[0,.20,0]);
    ...
    plate(this.head,frame,.56,.22,.06,[0,0,0],.82);
    for(const side of [-1,1]){
      const slit=block(this.head,sensor,[.18,.018,.018],[side*.14,.03,.05]);slit.rotation.z=side*.10;
      cylinder(this.head,steel,.043,.055,[side*.085,-.065,.04],"z");
      cylinder(this.head,frame,.028,.060,[side*.085,-.065,.045],"z");
      loft(this.torso,armor,[[-.55,.16,.84,.815],[.05,.22,.79,.765],[.55,.13,.60,.575]],[side*.39,.20,0]);
      // Swept side brow and red sensor channel, integrated into the shell.
      const brow=loft(this.torso,trim,[[-.64,.13,.16,-.04],[.34,.19,.12,-.03],[.85,.10,-.04,-.11]],[side*.79,.66,0]);
      brow.rotation.z=-side*.09;
      block(this.torso,frame,[.07,.07,.46],[side*.835,.61,.37]);
      block(this.torso,sensor,[.075,.017,.32],[side*.87,.618,.37]);
      plate(this.torso,armor,.34,.36,.19,[side*.78,.17,-.55],.86);
      for(let i=0;i<(fine?7:3);i++)block(this.torso,frame,[.33,.02,.035],[side*.53,.47-i*.04,-.93]);
      ...
      loft(this.torso,frame,[[-.30,.19,.20,-.11],[.15,.19,.14,-.11]],[side*.75,.89,-.54]);
      for(let row=0;row<3;row++)for(let col=0;col<4;col++){
        cylinder(this.torso,steel,.022,.026,[side*.75-.12+col*.08,.83+row*.07,-.375],"z");
        cylinder(this.torso,amber,.012,.029,[side*.75-.12+col*.08,.83+row*.07,-.372],"z");
      }
      for(let i=0;i<3;i++)block(this.torso,trim,[.028,.055,.015],[side*(.35+i*.06),.35,.96]);
    }
    // Reverse knees: upper link folds back; long shin returns forward to broad split toes.
    const makeLeg=(side:number):Leg=>{
      ...
      plate(hip,trim,.20,.36,.04,[side*.06,-.30,.232],.70);
      ...
      plate(knee,trim,.22,.77,.04,[side*.065,-.53,.225],.56);
      for(const edge of [-1,1]){
        cylinder(knee,frame,.067,.75,[edge*.20,-.51,-.035],"y");
        cylinder(knee,steel,.038,.86,[edge*.20,-.68,-.035],"y");
      }
      for(let i=0;i<(fine?6:3);i++)block(knee,frame,[.20,.020,.025],[0,-.25-i*.085,.255]);
      block(knee,amber,[.10,.028,.016],[side*.04,-.75,.258]);
      ...
      for(const toeSide of [-1,1]){
        const toe=loft(ankle,armor,[[-.05,.13,.06,-.08],[.43,.145,.0,-.095],[.60,.105,-.025,-.095]],[toeSide*.19,-.025,0]);
        toe.rotation.y=-toeSide*.09;
        block(ankle,frame,[.27,.045,.25],[toeSide*.23,-.102,.42]);
        block(ankle,steel,[.12,.025,.12],[toeSide*.23,-.04,.50]);
      }
    ...
    const makeArm=(side:number):Arm=>{
      ...
      block(elbow,trim,[.23,.035,.48],[0,.26,.25]);
      block(elbow,frame,[.19,.21,.34],[side*.25,-.035,.13],"feed_housing");
      strut(mantle,steel,.033,[0,-.15,-.05],[side*.11,-.32,.46]);
      for(let i=0;i<(fine?5:2);i++)block(elbow,steel,[.13,.022,.025],[side*.15,.13-i*.038,.55]);
      ...
      for(let i=0;i<3;i++)block(elbow,trim,[.05,.018,.025],[-.08+i*.08,.20,.65]);
    ...
    if(fine)for(const side of [-1,1]){
      for(let i=0;i<4;i++)cylinder(this.torso,steel,.012,.022,[side*(.33+i*.10),.41,.93],"z");
      for(const leg of [side<0?this.legs.left:this.legs.right])
        for(const y of [-.26,-.69])cylinder(leg.knee,steel,.013,.025,[side*.13,y,.233],"z");
    }
